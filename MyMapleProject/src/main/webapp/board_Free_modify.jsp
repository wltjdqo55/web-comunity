		...
		<%-- 가운데 content div --%>
			...
			<%--가운데 DIV --%>
				...
				<div class = "center_content">
					<form action = "BoardmodifyOK.do" method = "post" enctype="multipart/form-data">
					<%-- 글쓰기 폼 --%>
					<div class = "center_content_title">
						<div class = "select_left">
							<select name="category">
							  <option value="1" ${dto.category == '분류' ? 'selected' : ''}>분류</option>
							  <option value="2" ${dto.category == '수다' ? 'selected' : ''}>수다</option>
							  <option value="3" ${dto.category == '이벤트' ? 'selected' : ''}>이벤트</option>
							  <option value="4" ${dto.category == '웃긴글' ? 'selected' : ''}>웃긴글</option>
							  <option value="5" ${dto.category == '인사' ? 'selected' : ''}>인사</option>
							  <option value="6" ${dto.category == '정보' ? 'selected' : ''}>정보</option>
							  <option value="7" ${dto.category == '투표' ? 'selected' : ''}>투표</option>
							</select>
						</div>
