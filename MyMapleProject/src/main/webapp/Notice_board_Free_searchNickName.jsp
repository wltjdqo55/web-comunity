...
background-gradient: rgba(0, 0, 0, 0.5) 0%, rgba(0, 0, 0, 0) 100%;}
	tr:hover{background-color:#e3e4e6;}
 		...
 		.span_l{color : #d0d0d0; }
 		#span_text_entire{background-color: #666666; color : white;}
 		.span_text>a{text-decoration: none;}
 		#span_text_entire>a{color: white;}
 		#span_text_good>a{color : black;}
				...
				<div class = "center_content">
					...
					<div class = "board_free_Input">
						...
						<c:if test="${not empty sessionScope.userInfo}">
							...
							<span><a href = "Notice_onetwo_Board.do?good=10">👍10추글</a></span>
							<span><a href = "Notice_My_writing_board.do<%-- userId = ${sessionScope.userInfo.userId }--%>">내 글</a></span>
							<span><a href = "Notice_My_comment_board.do<%--userId = ${sessionScope.userInfo.userId}}--%>">내 댓글</a></span>
						</div>
						</c:if>
						...
						<div class = "menu_categori_choice">
							<span class = "span_text" id = "span_text_entire"><a href = "Notice.do">전체</a></span><span class = "span_l">|</span>
							<span class = "span_text" id = "span_text_good"><a href = "Notice1.do?good=1">공지</a></span><span class = "span_l">|</span>
							<span class = "span_text" id = "span_text_good"><a href = "Notice1.do?good=2">이벤트</a></span><span class = "span_l">|</span>
							<span class = "span_text" id = "span_text_good"><a href = "Notice1.do?good=3">점검</a></span><span class = "span_l">|</span>
							<span class = "span_text" id = "span_text_good"><a href = "Notice1.do?good=4">소식</a></span><span class = "span_l">|</span>
							<span class = "span_text" id = "span_text_good"><a href = "Notice1.do?good=5">버그제보</a></span><span class = "span_l">|</span>
							<hr>
						</div>
						...
						<c:forEach items="${mylist}" var="mylist">
						      <tr class = "trtrtr">
						        <td class = "bunhoTD"><c:out value="${mylist.num}" /></td>
						        <td class = "titleTD">
						        	<c:set var="countValue" value="${mylist.comment}" />
						        	<a href = "Notice_board_Free_Content_View.do?num=${mylist.num}&comment=${countValue}">
						        		<c:out value="[${mylist.category}]" /> 
						        		<c:out value="${mylist.userTitle}" />&nbsp;
						        		<c:if test="${mylist.comment ne 0}">
										  	<span class="zxs">[<c:out value="${mylist.comment}" />]</span>
										</c:if>
						        	</a>
						        	 <c:choose>
						        ...
						        <td class="NickNameTD">
								  <span class="user-nickname" onclick="showOptions(event)"><c:out value="${mylist.userNickName}" /></span>
								  <div class="option1">
								  	<div class = "option1-1">
								  		<a href="Notice_Board_Free_NickName_search.do?userNickName=${mylist.userNickName}">이름으로 검색</a>
								  	</div>
								  	<c:if test = "${empty sessionScope.userInfo}">
								  	<div class = "option1-2">
								  	...
								  	<c:if test = "${not empty sessionScope.userInfo}">
								  	<div class = "option1-2">
								  		 <a href="note_writingGood.do?userNickName=${mylist.userNickName}" onclick="window.open(this.href, '_blank', 'width=700,height=550,left=300, top=150'); return false;">쪽지 쓰기</a>	
								  	</div>
								  	</c:if>
								  </div>
								</td>
						        <td class = "wtimeTD">
						        	<script>
						        	 var wtime = new Date("<c:out value="${mylist.wtime}" />");
						             var today = new Date();
						        ...
						        <td class = "hitTD"><c:out value="${mylist.hit}" /></td>
						        <td class = "recommendTD"><c:out value="${mylist.recommend}" /></td>
						      </tr>
							</c:forEach>
							...
							<%--폼 메소드 끝--%>
							<div style = "float: right; width:30%;">
								<a href = "Notice.do"><span class = "board_free_list">목록</span></a>
								<c:if test = "${userInfo ne null }">
									<a href = "Notice_board_free_write.do" onclick="checkUserPower(event)"><span class = "board_free_write">글쓰기</span></a>
								</c:if>
								<c:if test = "${userInfo eq null }">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Notice_board_Free_NickName_search.do?curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
