...
.Modify_Board_Content{width: 100%; height: 100%; display: block;}
...
<body>
	...
	<div class="container">
		 <div class = "container_wrap">
		 	<div class = "container_wrap_box">
		 		...
		 		<div style = "width: 100%; height: 100px; margin-top: 30px; margin-left:20px; display: block; line-height: 2.3" class ="header_c">
		 ...
		 <div class = "container_warp_content">
		 	...
		 	<%--수정하기 --%>
		 	<div class = "Modify_Board_Content">
			 	<form action = "Admin_Notice_ModifyOK.do" method = "post" class = "myform4">
			 		<input type = "text" name = "title" class = "classB" style = "width: 84%; height: 44px; border: none;border-radius:10px; background-color:#e8f4ea; text-indent: 20px; font-size: 18px;" placeholder="제목을 입력해주세요." value = "${dto.title }">
			 		<input type = "submit" value = "공지 수정하기" class = "sub_btn4">
			 		 <textarea id="summernote4" name = "content">${dto.content }</textarea>
	...
	FreeBoard_span.addEventListener("click", function(){
		...
		document.querySelector(".Modify_Board_Content").style.display = "none";
	...
	const sub_btn4 = document.querySelector(".sub_btn4");
	sub_btn4.addEventListener("click", function(event){
		event.preventDefault();
		var confirm = window.confirm("공지된 게시글을 수정하시겠습니까?");
		if(confirm){
			 var myform4 = document.querySelector(".myform4");
		      myform4.submit(); // 각각의 form 요소에 대해 submit 동작 실행
		}else{
		}
	});
	...
	let isContentVisible = true; // 초기 상태는 숨겨진 상태로 설정
