...
<div class = "container">
		<div class = "container_header">
			<div style = "float:left; width:45%; margin-left:80px; margin-top:15px;">
				<form action = "Admin_Board_Search.do" method = "post" id="myForm">
			...
			<div style = "width:30%;">
				<input type = "radio" name = "rad" id = "radio1" checked>&nbsp; 자유게시판 : ${boardCount} 개<br>
				<input type = "radio" name = "rad" id = "radio2" onclick="navigateToLink1('Admin_Board1.do')">&nbsp; 직업게시판 : ${WarriorCount} 개<br>
				<input type = "radio" name = "rad" id = "radio3" onclick="navigateToLink('Admin_Board2.do')">&nbsp; 정보공유게시판 : ${NoticeCount} 개				
			</div>
		...
		<table>
			...
			<tr>
				...
				<td class = "TD2">
					<a href = "Admin_Board_Content_View.do?num=${item.num}"><c:out value="${item.category}" /></a>
				</td>
				<td class = "TD3">
					<a href = "Admin_Board_Content_View.do?num=${item.num}"><c:out value="${item.userNickName}" /></a>
				</td>
				<td class = "TD4">
					<a href = "Admin_Board_Content_View.do?num=${item.num}">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Admin_Board.do?curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
	...
	<div class = "container">
		...
		<table>
			...
			<tr>
				...
				<td class = "TD4">
					<a href = "Admin_Board_Content_View.do?num=${item.num}"><c:out value="${item.userTitle}" />
					<c:if test="${item.comment ne 0}">
						<span style = "color:red; font-weight: bold;">[<c:out value="${item.comment}" />]</span>
					</c:if>
					<c:choose>
...
<div class="pageP">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Admin_Board_Search.do?category=0&search=${search}&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
...
<div class="pageP">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Admin_Board_Search.do?category=1&search=${search}&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
		...
		<div class="pageP">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Admin_Board_Search.do?category=2&search=${search}&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
