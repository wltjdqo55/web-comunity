			...
			<%--가운데 DIV --%>
				...
				<div class = "center_content">
					<div class = "center_content_good">
						<span><a href = "board_Free.do">메이플스토리 인벤 자유게시판</a></span>
					</div>
					...
					<div class = "center_ccc">
						...
						<c:if test="${sessionScope.userInfo.userNickName eq item.userNickName}">
							<span class = "replydelete"><a href = "board_free_writeOK.do?num=${item.num}">수정</a>&nbsp; | &nbsp;<a href = "board_free_delete.do?num=${item.num }" class = "deleteGOTO">삭제</a>&nbsp; | &nbsp;<a href = "board_Free.do">목록</a>&nbsp; | &nbsp;<a href = "#">댓글(${item.comment })</a></span>
						</c:if>
						<c:if test="${sessionScope.userInfo.userNickName ne item.userNickName}">
							<span class = "replydelete"><a href = "board_Free.do">목록</a>&nbsp; | &nbsp;<a href = "#">댓글(${item.comment})</a></span>
					...
					<div class = "center_content_recomand">
						...
						<c:if test = "${not empty sessionScope.userInfo.userNickName}">
							<div class = "good_recomand">
								<a href="FreeBoard_recommend.do?bnum=${item.num}&commend=1&userNickName=${sessionScope.userInfo.userNickName}&comment=${item.comment}"
								   onclick="return confirm('추천하시겠습니까?');">
								  <span class = "recommend">👍<c:out value="${item.recommend}" /></span>
								</a>
							</div>
							<div class = "fuck_recomand">
								<a href="FreeBoard_recommend.do?bnum=${item.num}&commend=0&userNickName=${sessionScope.userInfo.userNickName}&comment=${item.comment}"
					...
					<div class = "listandgo">
					 	<span><a href = "board_Free.do">목록</a>&nbsp; | &nbsp;<a href = "#">댓글(${item.comment})</a></span>
					</div>
					<div class = "command_user">
						...
						<div class = "userCo">
							...
							<div class = "conta_b" style = "display: none;">
								...
								<c:if test = "${not empty sessionScope.userInfo}">
									...
									<input type = "button" value = "등록" class = "sub_r" onclick = "btn_good('${sessionScope.userInfo.userId}', '${sessionScope.userInfo.userNickName }', '${sessionScope.userInfo.userPassword }', '${sessionScope.userInfo.userName }',${item.num }, ${item.comment }, ${replylist.groupNum }, ${replylist.stepNum })">
								</c:if>
										</div>
									</c:if>
								</div>
						...
						<div class = "ckckck">
							<div class = "ckckck_left">
								<span><a href = "board_Free.do">목록</a></span>
								<span><a href = "NextPred_board_Free_Content_View.do?num=${item.num}&next=1&comment=${item.comment}">다음글</a></span>
								<span><a href = "NextPred_board_Free_Content_View.do?num=${item.num}&next=0&comment=${item.comment}">이전글</a></span>
							</div>
