...
<style>
	body{margin: 0;padding : 0; width:100vw; margin-bottom: 500px; margin-bottom: 150px;background-image: url('images/메이플 배경이미지2.jpg'); background-size: 100% 100%; background-repeat: no-repeat; background-position: 0% 100%;
background-attachment: fixed;
background-gradient: rgba(0, 0, 0, 0.5) 0%, rgba(0, 0, 0, 0) 100%;}
	
 		/*가운데 div */
 		.container_wrap{height: auto; background-color: white;}
 		.container_box{width: 100%; border: 0; padding: 0 ; height: auto; }
 		.350img{padding-right: 30px;}
 		.center_content_top_img{width : 62vw; height: 300px;}
 		.center_content{height: auto;}
 		...
 		.span_text{padding: 5px 10px 5px 10px; margin: 20px; border-radius: 10px; font-weight: bold; }
 		.span_l{color : #d0d0d0; }
 		.span_text>a{text-decoration: none;}
 		#span_text_good>a{color : black; font-weight: bold;}
 		
 		.menu_list_table_div{height: auto;}
 		...
 		.pageP{width: 59.5vw; text-align: center; height: 30px; margin-top: 30px; font-size: 15px; color: black;}
					...
					<div class = "board_free_Input">
						<div style = "display:flex;">
							...
							<div style = "float: right; width:30%;">
								<a href = "Warrior.do"><span class = "board_free_list">목록</span></a>
						...
						<c:if test="${not empty sessionScope.userInfo}">
							...
							<span><a href = "Warrior_onetwo_Board.do?good=10">👍10추글</a></span>
							<span><a href = "Warrior_My_writing_board.do<%-- userId = ${sessionScope.userInfo.userId }--%>">내 글</a></span>
							<span><a href = "Warrior_My_comment_board.do<%--userId = ${sessionScope.userInfo.userId}}--%>">내 댓글</a></span>
						</div>
						</c:if>
						...
						<div class = "menu_categori_choice">
							<span class = "span_text 1" id = "span_text_entire"><a href = "Warrior.do">전체</a></span><span class = "span_l">|</span>
							<span class = "span_text 2" id = "span_text_entire"><a href = "Warrior1.do?good=1">전사</a></span><span class = "span_l">|</span>
							<span class = "span_text 3" id = "span_text_entire"><a href = "Warrior1.do?good=2">마법사</a></span><span class = "span_l">|</span>
							<span class = "span_text 4" id = "span_text_entire"><a href = "Warrior1.do?good=3">궁사</a></span><span class = "span_l">|</span>
							<span class = "span_text 5" id = "span_text_entire"><a href = "Warrior1.do?good=4">도적</a></span><span class = "span_l">|</span>
							<span class = "span_text 6" id = "span_text_entire"><a href = "Warrior1.do?good=5">해적</a></span><span class = "span_l">|</span>
							<hr>
						</div>
						<c:if test = "${IZ eq '0'}">
						<div class = "menu_list_table_div">
							...
							<c:forEach items="${AdminNoticelist}" var="item">
						      <tr class = "trtrtr noticetr" style="background-color:#e3e4e6; font-weight: bold;">
						        <td class = "bunhoTD"><c:out value="${item.state}" /></td>
						        <td class = "titleTD">
						        	<a href = "Notice_Content_View.do?num=${item.num}">
						        		<c:out value="[${item.category}]" /> 
						        		<c:out value="${item.title}" />&nbsp;
						        	</a>
						        </td>
						       <td class="NickNameTD">
								  <span class="user-nickname"><c:out value="${item.adminNickName}" /></span>
								</td>
							...
							<c:forEach items="${list}" var="item">
						      <tr class = "trtrtr">
						        ...
						        <td class = "titleTD">
						        	...
						        	<a href = "Warrior_board_Free_Content_View.do?num=${item.num}&comment=${countValue}">
						        		<c:out value="[${item.category}]" /> 
						       ...
						       <td class="NickNameTD">
								  ...
								  <div class="option1">
								  	<div class = "option1-1">
								  		 <a href="Warrior_Board_Free_NickName_search.do?userNickName=${item.userNickName}">이름으로 검색</a>
								  	</div>
								  	<c:if test = "${empty sessionScope.userInfo}">
								  	<div class = "option1-2">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Warrior.do?curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
				...
				<c:if test = "${IZ eq '1'}">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Warrior.do?good=1&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
				...
				<c:if test = "${IZ eq '2'}">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Warrior.do?good=2&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
				...
				<c:if test = "${IZ eq '3'}">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Warrior.do?good=3&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
				...
				<c:if test = "${IZ eq '4'}">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Warrior.do?good=4&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
				...
				<c:if test = "${IZ eq '5'}">
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="Warrior.do?good=5&curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
...
const FreeBoardgoList = document.querySelectorAll(".FreeBoardgo");
FreeBoardgoList.forEach(function (element) {
