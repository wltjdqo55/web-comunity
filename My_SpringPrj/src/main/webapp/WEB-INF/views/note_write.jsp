	...
	<div class = "container_right_box">
		<div class = "container_right_inner_box">
			<div class = "right_header">
				<span class = "right_header_span"><span style = "font-size : 20px;">📬</span> ${sessionScope.userInfo.userNickName}님의 <span style ="color:#f37736;">보낸 편지함</span>입니다.</span>
			</div>
			...
			<table border = "1">
				...
				<tr>
					...
					<td class = "titleTD"><a href = "note_content_view?num=<c:out value="${list.num}" />&userNickName=<c:out value="${list.tosendNickName}" />"><c:out value="${list.title}" /></a></td>
					<td class = "tosendTD"><c:out value="${list.tosendNickName}" /></td>
						...
						<c:forEach var="i" begin="0" end="${totalPage}" step="1">
							<c:choose>
								...
								<c:when test="${i ne curPage}">
									<a href="note_write.do?curPage=${i}">${i+1}</a>
								</c:when>
							</c:choose> 		
						</c:forEach>
